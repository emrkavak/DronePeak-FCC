...
object Profiles {
    ...
    fun load(context: Context, fileName: String): Profile {
        val json = readProfileJson(context, fileName)
    ...
    fun load4g(context: Context, aircraftSerial: String): Profile {
        val json = readProfileJson(context, "4g.json")
        val obj = JSONObject(json)
    ...
    private fun readProfileJson(context: Context, fileName: String): String {
        val override = File(File(context.filesDir, "upstream_profiles"), fileName)
        if (override.isFile) {
            runCatching {
                return override.readText()
            }
        }
        return readAsset(context, "profiles/$fileName")
    }
