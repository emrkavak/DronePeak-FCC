...
DronePeak does not install APKs from `doesthings/FreeFCC`. FreeFCC stays as the upstream source remote. Users receive updates only from this repository's signed DronePeak GitHub Releases, so the DronePeak package id, UI, icon, language layer, and signing key stay intact.
...
- `app/src/main/java/com/dronepeak/app/UpdateChecker.kt`: keep DronePeak release repo as the APK update source.
- `app/src/main/res/drawable/dronepeak_icon.png`: keep the DronePeak icon.
...
Keep one release keystore forever. If the signing key changes, Android will not install updates over the existing DronePeak app.
